class Dot {
  ...
  Dot crossover(Dot mommy, Dot daddy) {
    int crossoverPoint = Math.round(random(brain.directions.length));
    Dot crossover = new Dot();
    for (int i = 0; i < brain.directions.length; i++) {
     crossover.brain.directions[i] = mommy.brain.directions[i]; 
    }
    for (int i = 0; i < crossoverPoint; i++) {
      crossover.brain.directions[i] = daddy.brain.directions[i];
    }
    return crossover;
  }
